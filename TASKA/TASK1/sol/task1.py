...
class Pinns:
    ...
    def initial_spatial_condition(self, t):
        return (self.Th - self.To) / (1 + torch.exp(-200*(t - 0.25))) + self.To   
    ...
    def compute_pde_residual(self, input_int):
        ...
        residual1 = grad_Tf_t + grad_Tf_x - 0.05 * grad_Tf_xx + 5 * (Tf - Ts)
        residual2 = grad_Ts_t - 0.08 * grad_Ts_xx - 6 * (Tf - Ts)
        #residual = grad_Tf_t + grad_Tf_x - 0.05 * grad_Tf_xx + 5 * (Tf.reshape(-1,1) - Ts.reshape(-1,1)) + grad_Ts_t - 0.08 * grad_Ts_xx - 6 * (Tf.reshape(-1,1) - Ts.reshape(-1,1))
    ...
    def compute_loss(self, inp_train_sb_0, output_sb_0_Tf, output_sb_0_Ts, inp_train_sb_L, output_sb_L_Tf, output_sb_L_Ts, inp_train_tb, output_tb_Tf, output_tb_Ts, inp_train_int, output_int_Tf, output_int_Ts, verbose=True):
        ...
        r_sb_Tf_0 = output_sb_0_Tf - Tf_pred_sb_0
        ...
        loss = torch.log10(loss_tsb + 0.3*loss_int)
    ...
    def plotting_assemble_datasets(self):
        ...
        plt.savefig(self.dir + "assemble_dataset_BC._" + f"model_{datetime.datetime.now()}" + ".png")
    ...
    def plottting_bc(self):
        ...
        exact_tb_Tf = self.initial_condition(input_tb[:, 1])
        exact_tb_Ts = self.initial_condition(input_tb[:, 1])



        err_sb_Tf_0 = (torch.mean((exact_sb_Tf_0-output_sb_0_Tf) ** 2) / torch.mean(exact_sb_Tf_0 ** 2)) ** 0.5 * 100
    ...
    def plotting_derivatives(self):
        ...
        err_grad_Tf_x_L = (torch.mean((grad_Tf_x_L) ** 2)) ** 0.5 * 100
        ...
        axs[0].set_title("Derivative_x Tf at x=L: " + str(round(err_grad_Tf_x_L.item(),2)) + "%")
        axs[1].set_title("Derivative_x Ts at x=0: " + str(round(err_grad_Ts_x_0.item(),2)) + "%")
        axs[2].set_title("Derivative_x Ts at x=L: " + str(round(err_grad_Ts_x_L.item(),2)) + "%")
    ...
    def submission_file(self):
        ...
        inputs = torch.stack([t, x], 1)

        outputs_Tf = self.approximate_solution(inputs)[:,0]
        outputs_Ts = self.approximate_solution(inputs)[:,1]

        outputs_Tf = outputs_Tf.detach().numpy()
        outputs_Ts = outputs_Ts.detach().numpy()

        t = data[:,0]
        x = data[:,1]

        combined_array = np.column_stack((t, x, outputs_Tf, outputs_Ts))

        np.savetxt(self.dir + "/SubmissionTask1_LUCA_SACCHI_" + f"model_{datetime.datetime.now()}" + ".txt", combined_array, delimiter=",", header="t,x,tf,ts")
...
pinn = Pinns(n_int, n_sb, n_tb, To, Th)
